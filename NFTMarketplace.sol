// Website: https://www.polygaj.finance
// Telegram: https://t.me/PolyGaj
...
contract NFTMarketplace is ERC721, Ownable {
    ...
    struct Nft {
        ...
        uint256 purchaseTokenID; // defines the token used to purchase the NFT
    ...
    function addNft(string memory uri, string memory description,  bool mintable, uint256 purchaseTokenID,  uint256 purchaseTokenAmount, uint256 mintCap, address admin) external onlyOwner() {
        nfts.push(Nft({
            uri: uri,
    ...
    function updateNft(uint256 nftID, string memory description,  bool mintable, uint256 purchaseTokenID,  uint256 purchaseTokenAmount, address admin) external returns (bool) {
        ...
        require(purchaseTokenID <= purchaseTokens.length - 1, "updateNft:: Purchase Token does not exist");
        nft.purchaseTokenID = purchaseTokenID;
    ...
    function purchaseNft(uint256 nftID) external returns (bool){
        ...
        IERC20 purchaseToken = purchaseTokens[nft.purchaseTokenID]; // Set the purchase token for the NFT.
        require(purchaseToken.balanceOf(msg.sender) >= nft.purchaseTokenAmount, "PurchaseNft:: Not enough tokens to make purchase");

        // Ensure an approval is done here.
        purchaseToken.transferFrom(msg.sender, burnAddress, nft.purchaseTokenAmount); // Transfer tokens from sender to contract, external fees WILL apply, so be careful with distribution.
    ...
    function getTokenBalance(uint256 purchaseTokenID) public view returns (uint256){
        IERC20 purchaseToken = purchaseTokens[purchaseTokenID];
